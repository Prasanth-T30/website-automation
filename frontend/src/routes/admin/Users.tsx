...
export default function Users() {
  ...
  const createUser = useMutation({
    mutationFn: (values: CreateValues) =>
      usersApi.create({ ...values, phone: values.phone || null }),
  ...
  const toggleActive = useMutation({
    mutationFn: (u: User) => usersApi.update(u.id, { is_active: !u.is_active }),
  ...
  const resetPassword = useMutation({
    mutationFn: (u: User) => usersApi.resetPassword(u.id).then((r) => ({ r, name: u.full_name })),
  ...
  const deleteUser = useMutation({
    mutationFn: (u: User) => usersApi.delete(u.id).then(() => u.full_name),
    ...
    <>
      ...
      >
        <p className="text-sm text-fg-secondary">
          ...
          {pendingDelete?.email}) will be permanently removed. If they own any records once
          students or batches are in use, deactivating is the safer choice.
        </p>
      </Dialog>
    </>
  );
