...
from app.api.deps import BatchRepo, CurrentUser, StudentRepo
...
def list_notifications(
    batches: BatchRepo, students: StudentRepo, user: CurrentUser
) -> list[NotificationOut]:
    batches.sync_lifecycle()
    owner_id = None if user.role is UserRole.admin else user.id
    return build_notifications(batches.list_all(), students.list_all(), owner_id=owner_id)
